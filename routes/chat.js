...
router.get("/api/searchFriends", VerifyToken, function (req, res) {
    ...
    var sql = "SELECT kayttaja.kayttaja_id, kayttaja.kuva, kayttaja.sahkoposti, kaverilista.vastaanottaja_id, kaverilista.lahettaja_id FROM kaverilista, kayttaja WHERE kaverilista.hyvaksytty = ? AND ((kaverilista.vastaanottaja_id = kayttaja.kayttaja_id AND kaverilista.lahettaja_id = ?) OR (kaverilista.lahettaja_id = kayttaja.kayttaja_id AND kaverilista.vastaanottaja_id = ?)) AND kayttaja.sahkoposti LIKE ?";



    (async () => { // IIFE (Immediately Invoked Function Expression)
        try {
            const rows = await query(sql,[1, req.userData.id, req.userData.id, '%' + req.query.search + '%']);
